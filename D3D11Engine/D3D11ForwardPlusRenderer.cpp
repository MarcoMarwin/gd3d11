...
void D3D11ForwardPlusRenderer::AddGeometryPasses(
    ...
    const bool useScreenSpaceShadowMask = rendererSettings.EnableShadows
        ...
        && engine.GetShadowMaps()->ShouldRenderCSMShadows();
    ...
    if ( useScreenSpaceShadowMask ) {
        graph.AddPass( RG_PASS_NAME("FP Shadow Mask"), [&]( RGBuilder& builder, RenderPass& pass ) {
            ...
            pass.m_executeCallback = [this, &engine, shadowMaskResource]( const RenderGraph& graph ) -> void {
                ...
                // Bind CSM depth plus the comparison sampler.
    ...
    graph.AddPass( RG_PASS_NAME("FP Lit Geometry"), [&]( RGBuilder& builder, RenderPass& pass ) {
        ...
        pass.m_executeCallback = [this, &engine, colorResource, normalsResource, specularResource, reactiveMaskResource, transparencyAndCompositionMaskResource, velocityBufferHandle, shadowMaskResource, useScreenSpaceShadowMask, fsr3MasksActive]( const RenderGraph& graph ) -> void {
