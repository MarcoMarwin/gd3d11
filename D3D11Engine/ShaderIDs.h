#pragma once
#include <cstddef>

enum class VShaderID : size_t {
    VS_Ex,
    VS_ExNode,
    VS_Decal,
    VS_ExWater,
    VS_ParticlePoint,
    VS_ParticlePointShaded,
    VS_ExWS,
    VS_ExSkeletal,
    VS_ExSkeletalVN,
    VS_TransformedEx,
    VS_TransformedEx_MAX_Z,
    VS_ExPointLight,
    VS_XYZRHW_DIF_T1,
    VS_XYZRHW_DIF_T1_MAX_Z,
    VS_ExInstancedObj,
    VS_ExInstanced,
    VS_Lines,
    VS_Lines_XYZRHW,
    VS_PFX,
    VS_CinemaScope,
    VS_AdvanceRain,
    VS_ExLayered,
    VS_ExNodeLayered,
    VS_ExSkeletalLayered,
    VS_ExCube,
    VS_ExNodeCube,
    VS_ExSkeletalCube,
    VS_ExNodeInstanced,
    VS_DecalInstanced,
    COUNT
};

enum class PShaderID : size_t {
    PS_Lines,
    PS_LinesSel,
    PS_Simple,
    PS_Simple_FF,
    PS_Rain,
    PS_Rain_Snow,
    PS_Transparency,
    PS_TransparencySkel,
    PS_World,
    PS_World_NoMV,
    PS_Water,
    PS_ParticleDistortion,
    PS_ParticleSimple,
    PS_ParticleSimple_FF,
    PS_PFX_ApplyParticleDistortion,
    PS_PFX_Simple,
    PS_PFX_Simple_R8,
    PS_PFX_AOComposite,
    PS_PFX_AOCompositeHybrid,
    PS_PFX_ReconstructNormal,
    PS_PFX_VelocityDebug,
    PS_PFX_GaussBlur,
    PS_PFX_BloomCombine,
    PS_PFX_Heightfog,
    PS_PFX_LowClouds,
    PS_PFX_LowCloudComposite,
    PS_PFX_UnderwaterFinal,
    PS_PFX_Alpha_Blend,
    PS_PFX_CinemaScope,
    PS_PFX_DistanceBlur,
    PS_PFX_LumConvert,
    PS_PFX_LumAdapt,
    PS_PFX_HDR,
    PS_PFX_GodRayMask,
    PS_PFX_GodRayZoom,
    PS_PFX_Tonemap,
    PS_AtmosphereGround,
    PS_Atmosphere,
    PS_AtmosphereOuter,
    PS_FixedFunctionPipe,
    PS_Video,
    PS_DS_PointLight,
    PS_DS_PointLightDynShadow,
    PS_DS_AtmosphericScattering,
    PS_Diffuse,
    PS_PortalDiffuse,
    PS_WaterfallFoam,
    PS_WaterMask,
    PS_DS_AtmosphericScattering_Rain,
    PS_LinDepth,
    PS_DiffuseNormalmapped,
    PS_DiffuseNormalmappedFxMap,
    PS_DiffuseAlphaTest,
    PS_DiffuseAlphaTestShadows,
    PS_WindowCutoutDepth,
    PS_DiffuseNormalmappedAlphaTest,
    PS_DiffuseNormalmappedAlphaTestFxMap,
    PS_Preview_White,
    PS_Preview_Textured,
    PS_Preview_TexturedLit,
    PS_PFX_Sharpen,
    PS_PFX_GammaCorrectInv,
    PS_PFX_DoF_FocusResolve,
    PS_PFX_DoF,
    PS_PFX_DoF_Gauss,
    PS_PFX_DoF_Composite,
    PS_PFX_Velocity,
    PS_PFX_SkyVelocity,
    PS_PFX_CAS,
    PS_PFX_Composition,
    PS_PFX_WetGroundSSR,
    PS_FP_Diffuse,
    PS_FP_DiffuseNormalmapped,
    PS_FP_DiffuseNormalmappedFxMap,
    PS_FP_DiffuseAlphaTest,
    PS_FP_DiffuseNormalmappedAlphaTest,
    PS_FP_DiffuseNormalmappedAlphaTestFxMap,
    PS_FP_ShadowMask,
    COUNT
};

enum class GShaderID : size_t {
    GS_VertexNormals,
    GS_Cubemap,
    GS_ParticleStreamOut,
    COUNT
};

enum class CShaderID : size_t {
    CS_AdvanceRain,
    CS_LightCulling,
    CS_TiledShading,
    CS_PFX_GodRayMask,
    CS_PFX_GodRayZoom,
    CS_PFX_GodRayVolumetric,
    CS_PFX_GodRayTemporal,
    CS_PFX_GodRayCombine,
    CS_PFX_DoF_FocusResolve,
    CS_PFX_DoF,
    CS_PFX_DoF_Gauss,
    CS_PFX_DoF_Composite,
    CS_PFX_XeGTAO_Prefilter,
    CS_PFX_XeGTAO_Low,
    CS_PFX_XeGTAO_Medium,
    CS_PFX_XeGTAO_High,
    CS_PFX_XeGTAO_Ultra,
    CS_PFX_XeGTAO_Denoise,
    CS_PFX_XeGTAO_DenoiseLast,
    CS_PFX_XeGTAO_HybridNear_Low,
    CS_PFX_XeGTAO_HybridNear_Medium,
    CS_PFX_XeGTAO_HybridNear_High,
    CS_PFX_XeGTAO_HybridNear_Ultra,
    CS_PFX_XeGTAO_HybridFar_Low,
    CS_PFX_XeGTAO_HybridFar_Medium,
    CS_PFX_XeGTAO_HybridFar_High,
    CS_PFX_Sharpen,
    CS_VobCull,
    CS_VobPatchArgs,
    CS_VobHiZCopy,
    CS_VobHiZReduce,
    COUNT
};
