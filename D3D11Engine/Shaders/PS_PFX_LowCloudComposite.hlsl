...
float4 PSMain( PS_INPUT Input ) : SV_TARGET
{
    ...
    float cloudAlpha = saturate(clouds.a);

    if (cloudAlpha > 0.001f && rainVeil > 0.0001f)
    {
        float3 cloudColor = clouds.rgb / max(cloudAlpha, 0.001f);
        cloudColor = lerp(cloudColor, scene.rgb, rainVeil * lerp(0.65f, 1.0f, nightTimeBlend));
        cloudAlpha *= 1.0f - rainVeil * lerp(0.08f, 0.22f, nightTimeBlend);
