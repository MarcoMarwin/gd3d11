...
### Build 203
- Korrekturpush: CBuffer/CPU-InverseView Architektur f�r Window-Cutouts. Die WorldPosition-Exporte aus den Vertex Shadern wurden zur�ckgerollt, stattdessen wird die Inverse View Matrix einmalig pro Frame auf CPU-Seite berechnet und in den CutoutConstants-CBuffer hochgeladen. Dies vermeidet Vertex-Shader-Aufbl�hungen und erzielt das gleiche Ergebnis robuster im Pixel Shader.
...
- Korrekturpush: OilLamp Emission Logic �berarbeitet. Bei der Zuweisung von Lichtern an �llampen (ConfigureAllPointlightShadowSources) werden nun statische Lichter im Suchradius bevorzugt. Nur wenn kein statisches Licht gefunden wird, greift das System auf das n�chste dynamische Licht zur�ck.
- Korrekturpush: MeshVisualInfo um WindowGlassBounds erweitert f�r pr�ziseres Bounding-Box-Tracking von Fensterscheiben.
- Korrekturpush: C++ Typecast-Fix in der Window-Cutout-Logik (Korrektur von BaseVisualInfo auf MeshVisualInfo per dynamic_cast).
- Korrekturpush: C++ XMMATRIX Transpose-Fixes. Gothic speichert Vob-Matrizen standardm��ig f�r Shader (Column-Major) transponiert ab. F�r Berechnungen auf der CPU (DirectXMath) m�ssen diese zur�ck-transponiert werden. Das korrigiert die Orientierung der Window-Cutout-Bounds und die Richtung der �llampen-Licht-Offsets. Zusaetzlich wurde der Y-Offset f�r �llampen-Schatten auf 20.0f reduziert.
- Korrekturpush: Ocean Water Drift gefixt (PS_Water.hlsl). Die Weltkoordinaten f�r die Distortion-Animation werden f�r den Ozean nun um 90 Grad gedreht, damit die Wellenbewegung in die korrekte Richtung verl�uft.
- Korrekturpush: City-Window Validierung & Counterparts (GothicAPI). Gegen�berliegende City_Window-Instanzen (innen/au�en) werden nun automatisch verkn�pft, um zu pr�fen, ob die Transparenz an dieser Stelle g�ltig ist (verhindert L�cher in W�nden, wo keine Cutouts existieren). In PS_Simple.hlsl wurde ein spezieller FFDATA-Pfad f�r den transparenten Teil erg�nzt.
- Korrekturpush: Feintuning der �llampen-Schatten und Ocean-Water-Drift. Der Y-Offset f�r �llampen-Schatten wurde wieder auf 50.0f und der Forward-Offset auf 25.0f angepasst. Die Ozean-Rotation in PS_Water.hlsl wurde um weitere 180 Grad gedreht (-Z, X), um die exakte Original-Laufrichtung zu treffen.
- Korrekturpush: City-Window Validierung extrem vereinfacht. Die fehleranf�lligen Sonden f�r Fu�b�den und Decken wurden entfernt. Die Fenster fallen nun standardm��ig auf transparent (fail open) zur�ck, es sei denn, eine Mehrheit der Abtastpunkte findet definitiv eine parallele Wand hinter dem Cutout.
- Korrekturpush: �llampen-Emission gelockert. Bei der Suche nach statischen oder dynamischen Lichtern f�r die �llampen-Emission wird nicht mehr streng gefiltert, ob das Licht schon von einer normalen Flamme "verbraucht" wurde, was zuvor dazu f�hrte, dass viele Lampen dunkel blieben. Es wurde auch ein Log hinzugef�gt, das die Emission-Links aufschl�sselt.
- Korrekturpush: Ocean Water Animation (PS_Water.hlsl) finalisiert. Statt die Koordinaten noch einmal umst�ndlich zu drehen, wird nun f�r den Ozean einfach die Animationszeit (RI_Time) umgekehrt (-1.0f). Dadurch laufen die Wellen exakt auf die K�ste zu, ohne dass sich die Form der Wellen verzieht oder Fl�sse beeintr�chtigt werden.
- Korrekturpush: Shader-Refactoring und Optimierungen. Die Window-Cutout-Schleife (PS_Diffuse.hlsl) nutzt nun 'firstbitlow' f�r noch schnellere Iterationen durch die Tile-Maske. Die �llampen-Emission wurde in einen dynamischen Branch ausgelagert und eine harte Discard-Regel f�r den Alphatest von City_Windows (auch im Forward-Path) eingef�hrt. Kleinere Aufr�umarbeiten im Water-Shader (Variablennamen).
- Korrekturpush: City-Window Backface-Culling anstatt Counterpart-Pairing (GothicAPI). Die umst�ndliche Logik, gegen�berliegende City_Window-Instanzen zu paaren, wurde komplett verworfen. Stattdessen liest die Engine nun direkt beim Parsen der Mesh-Geometrie die Front-Normale der realen Glasscheibe aus. In CollectVisibleVobs werden nach hinten zeigende Fenster (die untexturierte R�ckseite) nun direkt aussortiert und gar nicht erst in die Render-Queue aufgenommen. Das ist extrem elegant und performant!
- Korrekturpush: City-Window Backface-Culling Feintuning und RenderState-Refactoring. Das Backface-Culling f�r Fenster hat nun eine Toleranz von 5 Grad (BackfaceToleranceSinSq), um ein hartes Aufpoppen an scharfen Kanten zu vermeiden. In DrawFrameAlphaMeshes wurde das State-Management optimiert (CullMode f�r Glas auf CULL_NONE gesetzt, um falsche Winding-Orders von Innen-Meshes abzufangen, und redundante State-Updates eliminiert). Zus�tzlich wurden BSP-Tree-Guards eingef�gt, um Abst�rze in fr�hen Ladephasen von Gothic zu verhindern.
- Korrekturpush: City-Window Sky-Safeguard und Particle-Lighting (Nightly). In PS_Simple.hlsl wurde ein intelligenter Sky-Safeguard f�r City_Windows implementiert: �ber eine vertikale Probe (EvaluateWindowSkyPath) wird nun im Screen-Space gepr�ft, ob der Weg zum Himmel durch Weltgeometrie verdeckt ist; falls ja, blendet das Fenster auf opak �ber. In den Particle-Shadern wurde eine st�rkere n�chtliche Verdunkelung f�r Bodennebel (groundFog) integriert. Au�erdem schreibt opake Weltgeometrie nun korrekterweise WorldGeometryMask = 1.0f.
- Korrekturpush: City-Window Textur Update und Sky-Safeguard Verbesserungen. Die vertikalen Probes in PS_Simple.hlsl wurden von 8 auf 24 erh�ht, um keine Wandstreifen mehr zu �berspringen. Die DDS-Textur City_Window.dds wurde aktualisiert und der Shader erzwingt nun ein Minimum an Alpha (0.18f), damit das Glas immer sichtbar bleibt, selbst wenn die Textur voll transparent ist.
- Korrekturpush: Particle Night-Tint und Transparency-CBuffer. Bodennebel (und andere Partikel) nutzen nun GetAmbientNightWeight() und erhalten �ber ApplyAmbientNightTint eine echte n�chtliche T�nung. In PS_Transparency wurde zudem ein GA_LightingTint Parameter in den Constant Buffer aufgenommen.
- Korrekturpush: Legacy Waterfalls Hue-Correction (PS_Water.hlsl). Steile Wasserf�lle erhalten nun dieselbe Scene-Hue-Korrektur wie horizontales Wasser, allerdings erst nach der Reflection/Cubemap-Komposition, um Farbabweichungen in den Rot/Blau-Kan�len zu verhindern.
- Regul�rer Push: Abschlie�ende Optimierungen in Build 203. EvaluateWindowSkyPath bricht nun im [loop] fr�hzeitig ab, wenn das Ergebnis feststeht, und die horizontalen Probes werden nur ausgef�hrt, wenn der mittlere Pfad blockiert ist. Die Cache-Logik in GothicAPI nutzt std::atomic_bool. Atmospheric Scattering wird nun korrekt NACH der �llampen-Emission angewendet.
...
- Korrekturpush: Feintuning an WindowSkyVisibility und OilLamps. In D3D11GraphicsEngine wird das SRV nun explizit ungebunden, BEVOR der Compute-Shader aufgerufen wird, um D3D11 Hazard-Warnungen (gleichzeitiges Binden von SRV und UAV) zu beheben. Die SkyGuard-Distanz f�r Fenster wurde von 2000-3000 auf 1000-1500 reduziert, um den Effekt im Nahbereich schneller greifen zu lassen. F�r �llampen (GothicAPI) wurde die StrongChromaticSaturation von 0.55 auf 0.75 erh�ht, damit leicht gef�rbte Flammen �fter den wei�en Fallback nutzen.
- Regul�rer Push: Abschlie�ende Optimierungen in Build 204. Implementierung eines Textur-Swap-Mechanismus f�r Partikel (Dark/Bright Textures wie FIRESMOKE_DARK zu FIRESMOKE) in GothicAPI, um dynamische Verdunkelung zu unterst�tzen. �llampen mischen nun einmalig die Farben aus statischen und dynamischen Lichtquellen (MixOilLampEmissionColors), sodass die Lampen-Emission stabil bleibt und nicht mehr unangenehm mit Licht-Animationen flackert.

- Korrekturpush: Feintuning in Build 205. Die Output-Dithering-Kurve in PS_PFX_GammaCorrectInv wurde angepasst (Dithering bleibt nun �ber einen weiten Luminanzbereich aktiv), was Banding-Artefakte an W�nden und Laub extrem reduziert. In PS_ParticleDistortion und PS_ParticleSimple wurde die Tag/Nacht-Wichtung f�r Wasserpartikel optimiert. In PS_Simple wurde der alte Screen-Space Connectivity-Test f�r das untere Bildschirmdrittel der City_Windows verworfen, da Tiefe nahe Null dort dem Gothic-Void/Boden entspricht und nicht dem Himmel.
- Regul�rer Push: Abschlie�ende Optimierungen in Build 205. In PS_Water.hlsl wurde die Shore-Fade-Logik repariert (Wasser verblasst nun nicht mehr f�lschlicherweise in die leere Szene, wenn flaches Land dahinter liegt, und schmale Wasserf�lle werden nicht mehr weichgezeichnet). In ShadowSampling.h nutzt der Fallback-Pfad ohne Blue-Noise nun ebenfalls Hash-Rotation, um hartes Banding der PCF-Schatten zu verhindern. In WorldObjects.h wurden neue Properties (WaterGeometryClass) f�r das kommende Water-Refactoring vorbereitet.

- Korrekturpush: Auto Type Deduction Restore (Build 219)
  - **Auto Type Deduction:** Der Wechsel auf uto im Vegetation-Culling wurde auf ausdrücklichen Wunsch wiederhergestellt.
- Korrekturpush: Revert und Redesign in Build 206. Einige experimentelle Features des ersten Pushes (CSM-Schatten-Dither, Water-RenderMode-Split, Compute-Fallback in PS_Simple) wurden wieder verworfen. Daf�r wurde das Output-Dithering in PS_PFX_GammaCorrectInv deutlich nachgesch�rft: Es bleibt nun bis in die hellsten Bereiche (0.995f) aktiv, da Quantisierung auch in hellen, flachen Gradienten stark sichtbar war. Der Daytime Gothic-Fog (PS_PFX_Heightfog) nutzt nun weichere Fade-Grenzen f�r das Ausblenden der Geometrie und erzwingt im Hintergrund zwingend die Nebelfarbe, um unsch�ne blaue Silhouetten in der Ferne zu tilgen.
- Korrekturpush: Redesign der City-Window Sky-Protection (PS_Simple). Der Compute-Shader Ansatz (TX_WindowSkyVisibility) wurde verworfen und durch einen genialen, analytischen Texture-Space Raycast ersetzt. Die 5 Fensterreihen der City_Window Textur werden nun �ber Derivate (ddx, ddy) pr�zise in den Screen-Space projiziert, wodurch die Sichtbarkeit jeder Reihe unabh�ngig ermittelt werden kann. In ShadowSampling.h wurde das Penumbra-Dithering wieder entfernt und stattdessen eine dynamische Weichzeichnung bei flachen Einfallswinkeln (grazingFootprint) eingef�hrt. Zudem wurde in PS_World.hlsl der WorldGeometryMask Export aufger�umt.
- Korrekturpush: Perspektivisch korrekte Fensterprojektion & Nebel-Tuning. In PS_Simple.hlsl wurde die Screen-Space Projektion der Fenster-UVs massiv verbessert (WindowUvToScreenPosition rechnet nun mit UV/W statt linearen UVs, um Verzerrungen zu vermeiden). In PS_PFX_Heightfog.hlsl blockt Gothic-Nebel Geometrie in der Ferne nun unabh�ngig von der Tageszeit konsistent aus (worldFogOpacity greift das FarOcclusion-Gewicht auf). In ShadowSampling.h wurde das Grazing-Footprint-Experiment wieder verworfen.

- Regul�rer Push: Abschlie�ende Optimierungen in Build 207. Einf�hrung eines dynamischen Ocean-Climates: Je nach Welt (ADDONWORLD vs Khorinis) hat das Ozean-Wasser in PS_Water nun unterschiedliche Streu- und Absorptionsprofile sowie einen spezifischen Luma-neutralen Tint, der bei Regen und Nacht physikalisch korrekt abnimmt. In D3D11ShaderManager wurde die Shadow-Kernel-Qualit�t (PCF/PCSS Taps & Blue Noise) nun direkt an die TAA/FSR3-Rekonstruktion gekoppelt: Ohne TAA nutzt der Renderer deutlich mehr Taps und verzichtet auf stochastisches Noise, um Stippling auf animierten Charakteren zu vermeiden.

- Korrekturpush: Feinschliff in Build 208. Das Vegetation-Wind-System wurde perfektioniert (VS_ExInstancedObj.hlsl): Der Wind wird nun zwingend im World-Space nach der Instanz-Transformation angewendet, sodass sich Pflanzen bei Rotation im Spacer nicht mehr "in die falsche Richtung biegen". Das System nutzt nun zudem die reale Bodenneigung (WindGroundPlane aus WorldObjects.h), anstatt stur die lokale Bounding-Box f�r das Biegungsverhalten heranzuziehen. In PS_Water.hlsl fadet der Jharkendar-Ocean-Tint in flachem Wasser sanfter aus, damit die K�sten�berg�nge weich bleiben. F�r die PCSS-Schatten wurden die Tap-Zahlen (ShadowSampling.h) f�r das No-TAA-Fallback auf robustere Werte kalibriert.
- Korrekturpush: Weiterer Feinschliff in Build 208. In D3D11GraphicsEngine.cpp wurde die Zuweisung f�r die WindGroundPlane repariert (direkte float3 -> XMVECTOR Konvertierung statt XMLoadFloat3, um eventuelle Alignment-Probleme zu umgehen). In der Deferred Shading Pipeline (Legacy & Tiled) wurde ein genialer Kniff eingebaut: Der Distanz-Tier f�r die Pointlight-Schatten wird nun direkt in der ShadowSoftness encodiert (�ber einen 16.0f Marker). So k�nnen die Shader erkennen, ob sie weite Schatten mit weniger Taps rendern sollen, ohne dass der ohnehin hei�e Constant-Buffer wachsen muss!
- Korrekturpush: Abschlie�ender Wind-Polishing in Build 208. In VS_ExInstancedObj.hlsl (und D3D11GraphicsEngine.cpp) wurde zwischen "Gras" und "B�umen" unterschieden (abgeleitet aus VisualCamAlign). Gras-Patches nutzen nun eine affine Terrain-Scherung (signedTerrainShear), was die h�sslichen inversen Deformationen bei fehlerhafter Platzierung verhindert und den Boden-�bergang station�r h�lt. B�ume behalten ihr non-lineares Biegungsverhalten. Au�erdem wurde die Root-Gewichtung nun endlich auch auf die dynamische Charakter-Interaktion (Hero l�uft durch Busch) angewandt, sodass B�sche nicht mehr an der Wurzel vom Boden abrei�en!
- Korrekturpush: Performance- & Stabilit�ts-Update f�r das neue Wind-System (Build 208). In VS_ExInstancedObj.hlsl wurde f�r B�ume (grassShearProfile < 0.5f) wieder ein strikt rigider Stamm (untere 12%) erzwungen, damit Gel�nde-Ankerpunkte niemals Baumst�mme verschieben. Es wurde ein Distanz-LOD f�r den Wind eingef�hrt (windDistanceFade & detailLod), das hochfrequente Blatt-Turbulenzen auf Entfernung ausblendet und auf extreme Distanzen den Wind komplett abschaltet. Zur massiven Einsparung von ALU-Zyklen wird der vorherige Wind-Zustand (previousWorldWindOffset) f�r Motion Vectors nur noch berechnet, wenn ccurateWindVelocity (TAA/FSR) aktiv ist. In C++ wird die WindGroundPlane nun �ber WindGroundPlaneInitialized effizient gecacht.
